...
const pool = new Pool({connectionString: 'postgresql://neondb_owner:npg_Ti8uQUP7NRgA@ep-noisy-field-a5nhpuwg-pooler.us-east-2.aws.neon.tech/neondb?sslmode=require' });
...
async function createTables() {
  try {
    ...
    // Get the reader ID we just created
    const readerResult = await pool.query(`SELECT id FROM users WHERE username = 'mysticreader'`);
    ...
    console.log('\nFinal tables in the database:');
    finalTablesResult.rows.forEach((row, i) => {
      console.log(`${i+1}. ${row.table_name}`);
    });
    ...
    const livestreamsCount = await pool.query('SELECT COUNT(*) FROM livestreams');
