...
// Old database connection (we'll ask for this information)
const OLD_DATABASE_URL = 'postgresql://postgres:postgres@localhost:5432/soulseer';

// Set environment to use old database
process.env.DATABASE_URL = OLD_DATABASE_URL;

// Import with old database connection
...
async function migrateData() {
  console.log('Starting data migration from old database to new Neon database...');
...
async function migrateMessages() {
  try {
    ...
    const oldClient = await oldPool.connect();
    const { rows: messages } = await oldClient.query('SELECT * FROM messages');
    oldClient.release();
...
async function migrateReadings() {
  try {
    ...
    const oldClient = await oldPool.connect();
    const { rows: readings } = await oldClient.query('SELECT * FROM readings');
    oldClient.release();
...
async function migrateOrders() {
  try {
    ...
    const oldClient = await oldPool.connect();
    const { rows: orders } = await oldClient.query('SELECT * FROM orders');
    oldClient.release();
...
async function migrateOrderItems() {
  try {
    ...
    const oldClient = await oldPool.connect();
    const { rows: orderItems } = await oldClient.query('SELECT * FROM order_items');
    oldClient.release();
...
async function migrateLivestreams() {
  try {
    ...
    const oldClient = await oldPool.connect();
    const { rows: livestreams } = await oldClient.query('SELECT * FROM livestreams');
    oldClient.release();
...
async function migrateForumPosts() {
  try {
    ...
    const oldClient = await oldPool.connect();
    const { rows: posts } = await oldClient.query('SELECT * FROM forum_posts');
    oldClient.release();
...
async function migrateForumComments() {
  try {
    ...
    const oldClient = await oldPool.connect();
    const { rows: comments } = await oldClient.query('SELECT * FROM forum_comments');
    oldClient.release();
...
async function migrateGifts() {
  try {
    ...
    const oldClient = await oldPool.connect();
    const { rows: gifts } = await oldClient.query('SELECT * FROM gifts');
    oldClient.release();
...
async function migrateSessionLogs() {
  try {
    ...
    let sessionLogs;
    try {
      const { rows } = await oldClient.query('SELECT * FROM session_logs');
      sessionLogs = rows;
    } catch (err) {
...
async function migrateGiftLogs() {
  try {
    ...
    let giftLogs;
    try {
      const { rows } = await oldClient.query('SELECT * FROM gift_logs');
      giftLogs = rows;
    } catch (err) {
